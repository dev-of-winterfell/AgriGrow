...
class phoneAuthBuyerDetails : AppCompatActivity() {
    private lateinit var db: FirebaseFirestore
    private lateinit var auth: FirebaseAuth
    private lateinit var binding: ActivityPhoneAuthBuyerDetailsBinding
    private lateinit var googleSignInClient: GoogleSignInClient
    private lateinit var sharedPreferences: SharedPreferences
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityPhoneAuthBuyerDetailsBinding.inflate(layoutInflater)
        ...
        binding.signupbtn.setOnClickListener {
           ...
           //     val phoneNumber = binding.phonenumber.text.toString().trim()
                ...
                lifecycleScope.launch {
                    try {
                        val documents = db.collection("BUYERS").whereEqualTo("Email", email).get().await()
...
//    }
    ...
    private fun createUserDocumentInFirestore(email: String, userData: HashMap<String, String?>) {
        lifecycleScope.launch {
            try {
                val userDocument = db.collection("BUYERS").document(email)
                ...
                Toast.makeText(this@phoneAuthBuyerDetails, "Failed to register user: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private suspend fun createUserWithEmailAndPassword(email: String, password: String, user: Map<String, String>): Boolean {
        return try {
            ...
            if (task.user != null) {
                ...
                db.collection("BUYERS").document(email).set(userWithPhoto).await()
                true
            } else {
                false
            }
    ...
    private fun handleUserCreationResult(isSuccessful: Boolean, email: String, name: String) {
        if (isSuccessful) {
            ...
            FirebaseMessaging.getInstance().token.addOnCompleteListener { task ->
                ...
                val db = FirebaseFirestore.getInstance()
                val userRef = db.collection("BUYERS").document(userId)
